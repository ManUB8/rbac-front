...
const PosPage: React.FunctionComponent<IPosPageProps> = ({
    getValues,
    watch,
    setValue,
    errors,
    ...
    const { product_data, loading } = useLicenseProductFetch('2')
    const [open, setOpen] = React.useState(false);
    // ดึงเฉพาะ device = "pos"
    const productDevices = watch('product_device_permissions') ?? getValues('product_device_permissions') ?? [];
    const getPosIndex = (list: any[]) => list.findIndex(d => d.product_device_id === '2' || d.product_device_name === 'pos');
                ...
                <CardContent sx={{ pt: 2, pb: 0 }}>
                    ...
                    <Stack direction="row" alignItems="center" justifyContent="space-between" sx={{ px: 1 }}>
                        <Stack direction="row" spacing={1} alignItems="center">
                            ...
                            <Tooltip title="สิทธิ์การใช้งานและหน้าเมนูของอุปกรณ์ POS">
