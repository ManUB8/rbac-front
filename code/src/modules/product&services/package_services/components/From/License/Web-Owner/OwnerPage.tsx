...
const OwnerPage: React.FunctionComponent<IOwnerPageProps> = ({
    getValues,
    watch,
    setValue,
    errors,
    ...
    const { product_data, loading } = useLicenseProductFetch('1')
    const [open, setOpen] = React.useState(false);
    // ดึงเฉพาะ device = "pos"
    const productDevices = watch('product_device_permissions') ?? getValues('product_device_permissions') ?? [];
    const getPosIndex = (list: any[]) => list.findIndex(d => d.product_device_id === '1' || d.product_device_name === 'erp');
                ...
                <CardContent sx={{ pt: 2, pb: 0 }}>
                    ...
                    <Stack direction="row" alignItems="center" justifyContent="space-between" sx={{ px: 1 }}>
                        <Stack direction="row" spacing={1} alignItems="center">
                            <Typography variant="h6">{"Web Owner"}</Typography>
                            <Tooltip title="สิทธิ์การใช้งานของอุปกรณ์ Web Owner">
