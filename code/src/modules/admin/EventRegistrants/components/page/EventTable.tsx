...
import EventDialog from "../form/EventDialog";
...
const EventTable: React.FunctionComponent<IEventTableProps> = ({
    ...
    return (
        <>
                        ...
                        <Table stickyHeader>
                            ...
                            <TableBody>
                                {mastercontroller.event_loading ? (
                                    ...
                                    mastercontroller.event_data.map(
                                        (row, index) => (
                                            <TableRow
                                                key={row.student_activity_id}
                                                hover
                                                tabIndex={0}
                                            >
                                                {columns.map((column) => (
                                                    <TableCell
                                                        key={`${row.student_activity_id}-${column.id}`}
                                                        align={column.align}
                                                    >
                                                        {column.render(
                                                            row,
            ...
            <EventDialog mastercontroller={mastercontroller} />
        </>
    );
};
